    ...
    <header className="relative isolate overflow-hidden gradient-hero">
      <Container>
        <nav className="flex items-center justify-between py-5">
          ...
          <div className="hidden sm:flex items-center gap-4">
            <a href="#beneficios" className="text-buff/70 hover:text-buff">
              Benefícios
            </a>
            <a href="#como-funciona" className="text-buff/70 hover:text-buff">
              Como funciona
            </a>
            <a href="#contato" className="text-buff/70 hover:text-buff">
              Contato
            </a>
          </div>
        </nav>
      ...
      <Container>
        <div className="grid items-center gap-10 pb-16 pt-10 md:grid-cols-2 md:pb-24 md:pt-16">
          ...
          >
            <p className="text-beaver font-medium">
              Para quem já viu o básico, mas trava em começar
            </p>
            <h1 className="mt-2 text-4xl font-extrabold tracking-tight sm:text-5xl">
              Do básico à prática: mentoria para destravar sua programação
            </h1>
            <p className="mt-4 text-buff/80 leading-relaxed">
              Se você já viu variáveis, condicionais e laços — mas não sabe
              transformar isso em um
              <span className="font-semibold text-buff"> projeto real</span>, eu
              te guio passo a passo até a autonomia.
            </p>
          ...
          >
            ...
            <div className="pointer-events-none absolute -right-6 -top-6 hidden rotate-6 rounded-2xl border border-buff/30 p-3 text-buff/70 md:block">
              <span className="flex items-center gap-2 text-sm">
                <Rocket className="size-4 text-beaver" /> Bora construir de
                verdade
              </span>
            </div>
          </motion.div>
        ...
        <div className="grid items-center gap-10 md:grid-cols-[1.1fr_.9fr]">
          <motion.div className="order-2 md:order-1">
            ...
            <p className="mt-3 text-buff/80">
              Sou João Antônio, dev web em Belém (PA). Já passei pelo caminho do
              iniciante inseguro ao profissional — e transformei isso num método
              prático, direto ao ponto e sem enrolação.
            </p>
            <ul className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
              {[
                ...
                {
                  icon: LaptopMinimal,
                  label: "Ambiente (IDE/terminal) redondo",
                },
              ].map(({ icon: Icon, label }) => (
          ...
          >
            <div className="rounded-2xl border border-buff/20 bg-night p-6">
              <p className="text-sm uppercase tracking-wider text-buff/60">
                Sobre a mentoria
              </p>
              <h3 className="mt-2 text-xl font-semibold">
                Para quem já tocou no código e quer confiança
              </h3>
              <p className="mt-3 text-buff/80">
                A proposta é simples: sair do “exercício de sala” e construir
                algo útil. Você <strong>aprende a aprender</strong>, organiza
                seu estudo e ganha autonomia — sem se perder em mil tutoriais.
              </p>
            </div>
          </motion.div>
        </div>
  ...
  const items = [
    {
      icon: Compass,
      title: "Plano claro",
      desc: "Roteiro de estudo sob medida para o seu momento.",
    },
    {
      icon: Binary,
      title: "Do conceito ao projeto",
      desc: "Prática progressiva com desafios reais.",
    },
    {
      icon: Wrench,
      title: "Ferramentas na veia",
      desc: "IDE, Git, terminal — sem travar no setup.",
    },
    {
      icon: Users,
      title: "Apoio e comunidade",
      desc: "Você não estuda sozinho, compartilha progresso.",
    },
    {
      icon: ShieldCheck,
      title: "Confiança",
    ...
    },
    {
      icon: Rocket,
      title: "Autonomia",
      desc: "Método para seguir evoluindo depois da mentoria.",
    },
  ];
  return (
    <Section id="beneficios" className="bg-night/60">
      <Container>
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 auto-rows-fr">
          {items.map(({ icon: Icon, title, desc }) => (
            <motion.div
              key={title}
              className="rounded-2xl border flex items-center justify-start gap-3 border-buff/15 bg-night-2 p-5 shadow-[var(--shadow-soft)]"
              whileHover={{ y: -3 }}
      ...
      <Container>
        <div className="grid items-start gap-10 md:grid-cols-2">
          <div>
            ...
            <p className="mt-3 text-buff/80">
              Feita para iniciantes com alguma base escolar — como o Renato —
              que querem transformar o básico em prática.
            </p>
  ...
  const steps = [
    {
      ...
      desc: "Mapeamos seu momento, metas e lacunas.",
    },
    {
      icon: BookOpen,
      title: "Trilha sob medida",
      desc: "Plano de estudo com checkpoints semanais.",
    },
    {
      icon: Wrench,
      title: "Mão na massa",
      desc: "Aulas práticas, projetos e feedback contínuo.",
    },
    {
      icon: Rocket,
      title: "Autonomia",
      desc: "Método para seguir evoluindo sem se perder.",
    },
  ];
  return (
    <Section
      id="como-funciona"
      className="bg-[linear-gradient(180deg,var(--color-night-2),var(--color-night))]"
    >
      <Container>
